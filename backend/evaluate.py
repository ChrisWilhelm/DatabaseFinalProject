...
def main() -> None:
    ...
    for query, _ in queries2count:
        ...
        # plt.show()
    plt.plot(list(range(NUM_QUERIES)), median_ranks)
    plt.title("Median Rank over Website's Ordering w.r.t. Google News")
    plt.xlabel('Query Index')
    plt.ylabel('Median Rank')
    plt.plot(list(range(NUM_QUERIES)), intersection_sizes)
